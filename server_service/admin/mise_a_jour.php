...
$liste_etudiants = [];
foreach (sqlQueryAll($sql, [$_SESSION["rc"]["id_a"]]) as $i => $etd) {
...
try {
    ...
    foreach ($absents as $etd) {
        if (!in_array($etd, $liste_etudiants)) {
            sqlInsert($sql, [$etd, $_SESSION["rc"]["id_a"]]);
        }
    }
...
if (!empty($successMessage) || !empty($errorMessage)) {
    unset($_SESSION["rc"]["date"]);
    unset($_SESSION["rc"]["heure"]);
    unset($_SESSION["rc"]["minutes"]);
    unset($_SESSION["rc"]["matrix"]);
    unset($_SESSION["rc"]["id_a"]);
}
