...
if (isset($_POST['mise_a_jour'])) {
	...
	$_SESSION["rc"]["id_a"] = isset($_POST['index']) ? intval($_POST['index']) : "";

	// date, heure et minites
	$sql = "select date_heure from absence where id_a=?";
	$date_heure = sqlQuery($sql, [$_SESSION["rc"]["id_a"]]);
	$res = explode(" ", $date_heure["date_heure"]);
	$res2 = explode(":", $res[1]);

	$_SESSION["rc"]["date"] = $res[0];
	$_SESSION["rc"]["heure"] = $res2[0];
	$_SESSION["rc"]["minutes"] = $res2[1];
} else {
	if (!isset($_SESSION["rc"]["id_a"])) {
...
if (!empty($errorMessage)) {
	...
	if (isset($_POST['modifier'])) {
		if ($_SESSION["rc"]["type"] == "cm") {
			...
			$_SESSION["rc"]["matrix"] = sqlQueryAll($sql, [$_SESSION["rc"]["id_c"], $_SESSION["rc"]["id_a"]]);
		} else {
			$sql = "select etu.id_u, etu.prenom_nom , abs.id_a from" .
				"(select t1.id_u, CONCAT(t1.prenom,\" \",t1.nom) as prenom_nom from utilisateur t1 join etudiant t2 on t1.id_u=t2.id_e join ametice t3 on t2.id_e=t3.id_e " .
			...
			$_SESSION["rc"]["matrix"] = sqlQueryAll($sql, [$_SESSION["rc"]["id_c"], $_SESSION["rc"]["groupe"], $_SESSION["rc"]["id_a"]]);
		}
	}
		...
		<div class="card-body">
			<form method="POST" action="rechercher_cours.php">
				<div class="row g-3">
					...
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" id="date" class="form-control" name="date" placeholder="Date" value="<?php echo $_SESSION["rc"]["date"]; ?>" readonly="readonly">
							<label for="date">Date</label>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" id="heure" class="form-control" name="heure" placeholder="Heure" value="<?php echo $_SESSION["rc"]["heure"]; ?>" readonly="readonly">
							<label for="heure">Heure</label>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" id="minutes" class="form-control" name="minutes" placeholder="Minutes" value="<?php echo $_SESSION["rc"]["minutes"]; ?>" readonly="readonly">
				...
				<table class="table table-hover table-bordered mt-4">
					...
					<tbody>
						<?php
						if (!isset($_POST['mise_a_jour'])) {
							foreach ($_SESSION["rc"]["matrix"] as $abs) { ?>
								<tr>
									<?php
				...
				<div class="mt-2">
					<a href="rechercher_cours.php" class="btn btn-danger">Annuler</a>
