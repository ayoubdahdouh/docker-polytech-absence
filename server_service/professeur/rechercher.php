...
if (isset($_POST["index"])) {
	...
	$_SESSION["absence"]["id_c"] = $cours_id_c;
	$_SESSION["absence"]["type"] = $type;
	$_SESSION["absence"]["groupe"] = $groupe;

	$sql = "select nom from cours where id_c=?";
	$res = sqlQuery($sql, [$cours_id_c]);
	$cours_nom  = $res['nom'];

	$_SESSION["absence"]["cours_nom"] = $cours_nom;
	...
	if (count($res) != 0) {
		$_SESSION["absence"]["id"] = $res;
		$id_a = $_SESSION["absence"]["id"][0]["id_a"];
	} else {
...
if (!empty($warningMessage)) {
	...
	if ($_SESSION["absence"]["type"] == "cm") {
		...
		$matrix_abs = sqlQueryAll($sql, [$_SESSION["absence"]["id_c"], $id_a]);
	} else {
		$sql = "select etu.id_u, etu.prenom_nom , abs.id_a, abs.justificatif from" .
			"(select t1.id_u, CONCAT(t1.prenom,\" \",t1.nom) as prenom_nom from utilisateur t1 join etudiant t2 on t1.id_u=t2.id_e join ametice t3 on t2.id_e=t3.id_e " .
		...
		$matrix_abs = sqlQueryAll($sql, [$_SESSION["absence"]["id_c"], $_SESSION["absence"]["groupe"], $id_a]);
	}
				...
				<div class="row g-3">
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" name="cours_id_c" value="<?php echo $_SESSION["absence"]["id_c"]; ?>" hidden>
							<input type="text" class="form-control" id="cours" name="cours_nom" placeholder="Cours" value="<?php echo $_SESSION["absence"]["cours_nom"]; ?>" readonly="readonly">
					...
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" class="form-control" id="type" name="type" placeholder="Type" value="<?php echo $_SESSION["absence"]["type"]; ?>" readonly="readonly">
					...
					<div class="col-sm-4">
						<div class="form-floating">
							<input type="text" class="form-control" id="groupe" name="groupe" placeholder="Groupe" value="<?php if ($_SESSION["absence"]["groupe"] == 0) {
																																echo "Tous les groupes";
																															} else {
																																echo $_SESSION["absence"]["groupe"];
																															} ?>" readonly="readonly">
				...
				<div class="mt-3">
					...
					<div class="input-group" id="historique">
						<select class="form-select" id="historique" name="index">
							...
							foreach ($_SESSION["absence"]["id"] as $abs) {
								if ($abs["id_a"] ==	$id_a) {
